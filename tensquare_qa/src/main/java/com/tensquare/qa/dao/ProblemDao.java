...
public interface ProblemDao extends JpaRepository<Problem, String>, JpaSpecificationExecutor<Problem> {
  ...
  public Page<Problem> newListByLabelId(String labelId, Pageable pageable);
  ...
  public  Page<Problem> hotListByLabelId(String labelId,Pageable pageable);
  ...
  @Query(value = "SELECT * FROM tb_problem WHERE id IN (SELECT problemid FROM tb_pl WHERE labelid=?) AND reply=0 ORDER BY createtime DESC",nativeQuery = true)
    public  Page<Problem> waitListByLabelId(String labelID,Pageable pageable);
